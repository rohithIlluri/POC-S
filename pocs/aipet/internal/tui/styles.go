...
var (
	...
	cDanger  = lipgloss.AdaptiveColor{Light: "124", Dark: "203"} // over budget, errors, critical health
...
func faceStyle(m mood) lipgloss.Style {
	switch m {
	case moodWorried:
		return lipgloss.NewStyle().Bold(true).Foreground(cWarn)
	case moodThinking:
		return lipgloss.NewStyle().Foreground(cMuted)
	default:
		return lipgloss.NewStyle().Bold(true).Foreground(cSuccess)
	}
}
